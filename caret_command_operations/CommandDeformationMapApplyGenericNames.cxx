 ...
 * constructor.
 */
CommandDeformationMapApplyGenericNames::CommandDeformationMapApplyGenericNames()
   : CommandBase("-deformation-map-apply-generic-names",
                 "DEFORMATION MAP APPLY GENERIC NAMES")
{
}
 ...
 */
void
CommandDeformationMapApplyGenericNames::getScriptBuilderParameters(ScriptBuilderParameters& paramsOut) const
{
   ...
   paramsOut.addFile("Output Data File Name",
                     FileFilters::getAnyFileFilter());
   paramsOut.addDirectory("Source Directory Name");
   paramsOut.addFile("Source Spec File Name",
                     FileFilters::getSpecFileFilter());
   paramsOut.addFile("Source Closed Topology File Name",
                     FileFilters::getTopologyClosedFileFilter());
   paramsOut.addFile("Source Spherical Coordinate File Name",
                     FileFilters::getCoordinateSphericalFileFilter());
   paramsOut.addFile("Source Deformed Spherical Coordinate File Name",
                     FileFilters::getCoordinateSphericalFileFilter());
   paramsOut.addFile("Source Cut Topology File Name",
                     FileFilters::getTopologyCutFileFilter());
   paramsOut.addFile("Source Deformed Cut Topology File Name",
                     FileFilters::getTopologyCutFileFilter());
   paramsOut.addDirectory("Target Directory Name");
   paramsOut.addFile("Target Spec File Name",
                     FileFilters::getSpecFileFilter());
   paramsOut.addFile("Target Closed Topology File Name",
                     FileFilters::getTopologyClosedFileFilter());
   paramsOut.addFile("Target Spherical Coordinate File Name",
                     FileFilters::getCoordinateSphericalFileFilter());
   paramsOut.addFile("Target Cut Topology File Name",
                     FileFilters::getTopologyCutFileFilter());
   paramsOut.addFile("Output Spec File Name",
                     FileFilters::getSpecFileFilter());
}
 ...
 */
QString
CommandDeformationMapApplyGenericNames::getHelpInformation() const
{
   QString helpInfo =
      (indent3 + getShortDescription() + "\n"
       ...
       + indent9 + "<source-directory-name>\n"
       + indent9 + "<source-spec-file-name>\n"
       + indent9 + "<source-closed-topology-file-name>\n"
       + indent9 + "<source-spherical-coordinate-file-name>\n"
       + indent9 + "<source-deformed-spherical-coordinate-file-name>\n"
       + indent9 + "<source-cut-topology-file-name>\n"
       + indent9 + "<source-deformed-cut-topology-file-name>\n"
       + indent9 + "<target-directory-name>\n"
       + indent9 + "<target-spec-file-name>\n"
       + indent9 + "<target-closed-topology-file-name>\n"
       + indent9 + "<target-spherical-coordinate-file-name>\n"
       + indent9 + "<target-cut-topology-file-name>\n"
       + indent9 + "<output-spec-file-name>\n"
       + indent9 + "\n"
 ...
 */
void
CommandDeformationMapApplyGenericNames::executeCommand() throw (BrainModelAlgorithmException,
                                     CommandException,
                                     FileException,
                                     ...
                                     StatisticException)
{
   ...
   const QString sourceDirectoryName =
      parameters->getNextParameterAsString("Source Directory Name");

   const QString sourceSpecFileName =
      parameters->getNextParameterAsString("Source Spec File Name");

   const QString sourceClosedTopologyFileName =
      parameters->getNextParameterAsString("Source Closed Topology File Name");

   const QString sourceSphericalCoordinateFileName =
      parameters->getNextParameterAsString("Source Spherical Coordinate File Name");

   const QString sourceDeformedSphericalCoordinateFileName =
      parameters->getNextParameterAsString("Source Deformed Spherical Coordinate File Name");

   const QString sourceCutTopologyFileName =
      parameters->getNextParameterAsString("Source Cut Topology File Name");

   const QString sourceDeformedCutTopologyFileName =
      parameters->getNextParameterAsString("Source Deformed Cut Topology File Name");

   const QString targetDirectoryName =
      parameters->getNextParameterAsString("Target Directory Name");

   const QString targetSpecFileName =
      parameters->getNextParameterAsString("Target Spec File Name");

   const QString targetClosedTopologyFileName =
      parameters->getNextParameterAsString("Target Closed Topology File Name");

   const QString targetSphericalCoordinateFileName =
      parameters->getNextParameterAsString("Target Spherical Coordinate File Name");

   const QString targetCutTopologyFileName =
      parameters->getNextParameterAsString("Target Cut Topology File Name");

   const QString outputSpecFileName =
   ...
   //
   // Update names in the deformation map file
   //
   deformationMapFile.setSourceClosedTopoFileName(sourceClosedTopologyFileName);
   deformationMapFile.setSourceCutTopoFileName(sourceCutTopologyFileName);
   deformationMapFile.setSourceDeformedSphericalCoordFileName(sourceDeformedSphericalCoordinateFileName);
   deformationMapFile.setSourceDirectory(sourceDirectoryName);
   deformationMapFile.setSourceSpecFileName(sourceSpecFileName);
   deformationMapFile.setSourceSphericalCoordFileName(sourceSphericalCoordinateFileName);
   deformationMapFile.setTargetClosedTopoFileName(targetClosedTopologyFileName);
   deformationMapFile.setTargetCutTopoFileName(targetCutTopologyFileName);
   deformationMapFile.setTargetDirectory(targetDirectoryName);
   deformationMapFile.setTargetSpecFileName(targetSpecFileName);
   deformationMapFile.setTargetSphericalCoordFileName(targetSphericalCoordinateFileName);
   deformationMapFile.setOutputSpecFileName(outputSpecFileName);
   ...
   switch(dft) {
      ...
      case BrainModelSurfaceDeformDataFile::DATA_FILE_COORDINATE_FLAT:
         BrainModelSurfaceDeformDataFile::deformFlatCoordinateFile(
                                 ...
                                 deformedFileName,
                                 sourceDeformedCutTopologyFileName,
