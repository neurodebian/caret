                                      ...
                                      std::vector<QString>& tokens)
{
   ...
   bool done = false;
   while (done == false) {
      const int pos = s.find(separatorString);
      if (pos != -1) {
...
StringUtilities::convertURLsToHyperlinks(const QString& sIn)
{
   ...
   if (sIn.find("http://") == -1) {
      ...
      while(done == false) {
         ...
         //
         const int httpStart = s.find("http://", startPos); 
...
/**
 * Convert an string to a double.  Handles "em" or "en" dash (various width dashes/minus signs)
 */
double
StringUtilities::toDouble(const QString& sin)
{
   ...
   for (int mm = 0; mm < s.length(); mm++) {
      QCharRef ch = s[mm];  // "ch" is a reference to s[mm]
      //
      // Is this one of the "en" or "em" dash
      //
      if ((ch.unicode() >= 8208) && (ch.unicode() <= 8213)) {
         ch = '-';
      }
   }
   QString ss(s);
   return StringUtilities::toDouble(ss);
}
