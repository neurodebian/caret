...
FileUtilities::findFilesInDirectory(const QString& dirName, 
                                    const QString& regExpMatch,
                                    std::vector<QString>& matchingFiles)
{
   QDir dir(dirName);
   QStringList files = dir.entryList(regExpMatch,
                   ...
                   QString& result)
{
   ...
   StringUtilities::token(QDir::cleanDirPath(otherPathIn),
               "/\\",
               otherPath);
               
   std::vector<QString> myPath;
   StringUtilities::token(QDir::cleanDirPath(myPathIn),
               "/\\",
               myPath);
                        ...
                        const bool verboseMode)
{
   ...
   if (inputFileInfo.absFilePath() == outputFileInfo.absFilePath()) {
   ...
   std::ifstream in(inputName, std::ios::in | std::ios::binary);
   if (!in) {
...

{
   ...
   //
   const QString hemisphereRE("([L|R|LR]\\.)");
   ...
   //
   const int numberOfRegularExpresions = 6;
   ...
   if (validName) {
      ...
      if (description.isEmpty() == false) {
         ...
         for (int i = 0; i < numTypeNames; i++) {
            const int pos = descriptionLowerCase.find(typeNames[i]);
            if (pos != -1) {
               descriptionNoTypeName.setLength(pos);
               if (StringUtilities::endsWith(descriptionNoTypeName, ".")) {
                  descriptionNoTypeName.setLength(descriptionNoTypeName.length() - 1);
               }
               break;
            }
         }
...
FileUtilities::createDirectory(const QString& dirPathIn)
{
   ...
   //
   const QString savedDirectory = QDir::currentDirPath();
   ...
   while (dirStack.empty() == false) {
      ...
      if (dir.mkdir(dirName) == false) {
         std::cout << "Unable to create directory " << dirName.toAscii().constData() << " in " << QDir::currentDirPath().toAscii().constData()
                   << std::endl;
         return false;
      }
